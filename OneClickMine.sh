...
${sepa}
  或手动输入地址 by manual
  "
  echo && stty erase '^H' && read -p " pool.supportxmr.com:5555 Pool_address (默认 pool.supportxmr.com:3333 ):" x_address_num
    case ${x_address_num} in
        1)
        x_address=pool.supportxmr.com:5555
        ;;
        2)
...
set_xmr(){
    set_http_port
    set_pool_address
    stty erase '^H' && read -p " monero7 ):" x_currency
    [[ -z ${x_currency} ]] && x_currency=monero7
    stty erase '^H' && read -p "4AggMSAnFrQbp2c6gb98BZDBydgmjNhhxdN4qBSjwc3BDNQZwg5hRqoNczXMr7Fz6ufbwVaJL8sJmTdrp2dbKYcFRY2pe33):" x_username
    [[ -z ${x_username} ]] && echo " Error, 钱包地址未输入!" && exit 1
    stty erase '^H' && read -p " x ):" x_passwd
    stty erase '^H' && read -p " ck ):" x_id
...
Install_xmr(){
    ...
    clear
    mkdir ${file}
    git clone https://github.com/sbkcbig/xmr-wakuang.git ${file}
...
Limit_xmr(){
    ...
    if [[ -n ${PID} ]]; then
        ...
        stty erase '^H' && read -p "70" x_limit
